...
import charles_lun.loopview.indicator.PageIndicator;
import charles_lun.loopview.indicator.RecyleAdapter;
import charles_lun.loopview.salvage.RecyclingPagerAdapter;
import charles_lun.loopview.slideview.BaseSliderView;
import charles_lun.loopview.view.DepthPageTransformer;
import charles_lun.loopview.view.FixedScroller;
import charles_lun.loopview.view.MyViewPager;
...
//    private boolean isBorderAnimation = true;
    ...
    public InfiniteIndicatorLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mViewPager.setParentView((ViewGroup) mViewPager.getRootView());
    ...
    public <T extends BaseSliderView> void removeAllSliders() {
        mRecyleAdapter.removeAllSliders();
    }

    public <T extends BaseSliderView> void removeSlidersAt(int position) {
        mRecyleAdapter.removeSliderAt(position);
    }
    ...
    public void scrollOnce() {
        ...
        try {
            ...
            FixedScroller mScroller = new FixedScroller(mViewPager.getContext(), new AccelerateInterpolator());
            mScroller.setmDuration(150);
...
private int myDownX,myDoenY;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if(ev.getAction()==MotionEvent.ACTION_DOWN){
            ...
            int dx = (int) Math.abs(ev.getX() - myDownX);
            int dy = (int) Math.abs(ev.getY() - myDoenY);
            if (dx > dy && dx > 0) {
                //Log.i("log", "横划！拦截它");
                getParent().requestDisallowInterceptTouchEvent(true);
            } else if (dy > dx) {
   ...
   /* public void setIndicatorPosition(){
        setIndicatorPosition(IndicatorPosition.Right_Bottom);
    }*/
